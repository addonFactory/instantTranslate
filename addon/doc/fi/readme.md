...
Tätä lisäosaa käytetään valitun ja/tai leikepöydällä olevan tekstin kääntämiseen kielestä toiselle.
Käännös suoritetaan Google-kääntäjä-palvelua käyttäen.

## Kielten määrittäminen ##
Määritä lähde-, kohde- ja vaihdettava kieli valitsemalla "Pikakääntäjä..."-vaihtoehto kohdasta NVDA-valikko -> Mukautukset.
...
Tämä asetus on oikeastaan tarkoitettu vain tilapäiseen käyttöön. Mikäli tällä kokeellisella toiminnolla ei ole vaikutusta, kokeile vaihtaa manuaalisesti vakaisiin asetuksiin alla kuvailtua kielenvaihtokomentoa käyttäen. Toiminto on kokeellinen, koska Google ei tunnista todellista lähdekieltä oikein joissakin tilanteissa (tyypillisesti lyhyiden tekstien kanssa), ja sinun täytyy vaihtaa kieliä manuaalisesti näppäinkomentoa käyttäen pakottaaksesi lähdekieleksi aiemman kohdekielen (meidän tapauksessamme englanti).

Voit halutessasi valita "Vaihda kieltä automaattisesti, jos mahdollista" -asetuksen Puheasetukset-asetuspaneelista (NVDA-valikko -> Mukautukset -> Asetukset -> Puhe). Tällä tavoin syntetisaattori puhuu käännöksen kohdekielen äänellä, mikäli käytät monikielistä syntetisaattoria.

## Käyttö ##
Voit käyttää tätä lisäosaa kolmella eri tavalla:

1. Valitse tekstiä valitsemiskomennoilla
   (esim. Vaihto+nuolinäppäimet). Käännä sitten valittu teksti painamalla
...
## Pikanäppäimet ##
Seuraavia komentoja  on painettava toimintonäppäimen NVDA+Vaihto+T jälkeen:

* T: Käännä valittu teksti.
* Vaihto+T: Käännä leikepöydällä oleva teksti.
* S: Vaihda lähde- ja kohdekielet keskenään.
...
* V: Ota käyttöön automaattinen puhetulosteen kääntäminen tai poista se käytöstä.
...
* Pikanäppäinten käyttötapaa on muutettu. Paina ensin Pikakääntäjän
  toimintonäppäintä (NVDA+Vaihto+T) ja sitten haluamaasi toimintoa vastaavaa
