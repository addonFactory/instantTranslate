...
TranslationResult = namedtuple(
    "TranslationResult", ("translation", "langTo", "detectedLanguage")
)
...
TranslationProgress = namedtuple(
    "TranslationProgress", ("done", "total", "remaining", "percent")
)

LayerCommand = namedtuple(
    "LayerCommand", ("keys", "label", "scriptName", "description")
)
...
class ProgressBeeper:
    ...
    def start(self):
        ...
        threading.Thread(
            target=tick, daemon=True, name="InstantTranslateProgressBeeper"
        ).start()
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    @scriptHandler.script(
        description=_(
            "Instant Translate layer commands. Then press h to list available commands."
        )
    )
    ...
    def getLayerHelp(self):
        items = "".join(
            ...
            % (escape(command.label), escape(command.description))
            for command in getLayerCommands(type(self))
        )
    ...
    def translationSucceeded(self, key, translator):
        result = TranslationResult(
            translator.translation, translator.langTo, translator.detectedLanguage
        )
    ...
    def announceTranslation(self, result):
        ...
        queueHandler.queueFunction(
            ...
            lambda: executeWithSpeakOnDemand(messageWithLangDetection, msgTranslation),
        )
    ...
    def translateAndCache(self, text, langFrom, langTo, langSwap=None):
        if langFrom != "auto" and langSwap is not None:
            raise RuntimeError(
                "Unexpected arguments: langFrom={}, langTo={}, langSwap={}, text={}".format(
                    langFrom, langTo, langSwap, text
                )
            )
        ...
        if cached is not None:
            return cached
        myTranslator = self.startTranslator(
            self.translatorClass(langFrom, langTo, text, langSwap)
        )
        myTranslator.join()
        ...
        result = TranslationResult(
            myTranslator.translation, myTranslator.langTo, myTranslator.detectedLanguage
        )
        self.cacheStore(key, result)
        return result
    ...
    def announceProvider(self):
        ui.message(
            ...
            _("Using service: {provider}").format(
                provider=self.translatorClass.providerName
            )
        )
    ...
    def _localSpeak(self, sequence, *args, **kwargs):
        ...
        if self.autoTranslate and text_items:
            ...
            try:
                result = self.translateAndCache(
                    text_to_translate, self.lang_from, self.lang_to
                )
    ...
    def script_translationStatus(self, gesture):
        ...
        ui.message(
            ...
            _(
                "Translating: {done} of {total} parts done, {remaining} remaining, {percent}%"
            ).format(
                done=progress.done,
...
def makeLanguagePairScript(slot):
    ...
    return layerScript(
        ...
        description=_(
            "Switches to the language pair {number} configured in the settings."
        ).format(number=slot + 1),
        **speakOnDemand,
    )(script)
