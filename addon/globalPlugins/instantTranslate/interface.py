...
class InstantTranslateSettingsPanel(SettingsPanel):
    ...
    def makeSettings(self, sizer):
        ...
        helpLabel = wx.StaticText(
            self, label=_("Select translation source and target language:")
        )
        sizer.Add(helpLabel)
        ...
        self._providerChoice = helper.addLabeledControl(
            ...
            wx.Choice,
            choices=[provider.providerName for provider in PROVIDERS.values()],
        )
        self._providerChoice.Select(
            max(
                0,
                list(PROVIDERS).index(self.provider)
        ...
        self._fromChoice = helper.addLabeledControl(
            fromLabelText, wx.Choice, choices=sourceChoices(self.sourceLangs)
        )

        # Translators: A setting in addon settings dialog.
        intoLabelText = _("Target language:")
        self._intoChoice = helper.addLabeledControl(
            intoLabelText, wx.Choice, choices=list(self.targetLangs)
        )

        # Translators: A setting in addon settings dialog, shown if source language is on auto.
        swapLabelText = _("Language for swapping:")
        self._swapChoice = helper.addLabeledControl(
            swapLabelText, wx.Choice, choices=list(self.targetLangs)
        )
        self._fromChoice.Bind(wx.EVT_CHOICE, self.onFromSelect)

        # Translators: A setting in addon settings dialog, shown if source language is on auto.
        self.autoSwapChk = helper.addItem(
            wx.CheckBox(
                self,
                label=_(
                    "Activate the auto-swap if recognized source is equal to the target (experimental)"
                ),
            )
        )
        ...
        # Translators: A setting in addon settings dialog.
        self.copyTranslationChk = helper.addItem(
            wx.CheckBox(self, label=_("Copy translation result to clipboard"))
        )
        self.copyTranslationChk.SetValue(self.addonConf["copytranslatedtext"])

        # Translators: A setting in addon settings dialog.
        self.replaceUnderscores = helper.addItem(
            wx.CheckBox(
                self,
                label=_(
                    "Replace underscores with spaces (May provide better translation results depending on context)"
                ),
            )
        )
        ...
        # Translators: A setting in addon settings dialog.
        self.progressBeepsChk = helper.addItem(
            wx.CheckBox(self, label=_("Beep while a translation is in progress"))
        )
        self.progressBeepsChk.SetValue(self.addonConf["progressbeeps"])

        # Translators: A setting in addon settings dialog.
        self.announceStartChk = helper.addItem(
            wx.CheckBox(self, label=_("Announce when a translation starts"))
        )
        ...
        # Translators: A setting in addon settings dialog.
        self.donateBtn = helper.addItem(
            wx.Button(self, label=_("Support an author..."))
        )
        self.donateBtn.Bind(wx.EVT_BUTTON, self.onDonate)

        iLang_from = self._fromChoice.FindString(
            getName(self.sourceLangs, self.addonConf["from"])
        )
        iLang_to = self._intoChoice.FindString(
            getName(self.targetLangs, self.addonConf["into"])
        )
        iLang_swap = self._swapChoice.FindString(
            getName(self.targetLangs, self.addonConf["swap"])
        )
    ...
    def onLanguagePairs(self, evt):
        dialog = LanguagePairsDialog(
            self, self.languagePairs, self.sourceLangs, self.targetLangs
        )
        try:
...
class LanguagePairsDialog(wx.Dialog):
    def __init__(self, parent, pairs, sourceLangs, targetLangs):
        ...
        listLabel = _(
            "Language &pairs, activated by the keys 1 to 0 of the Instant Translate layer:"
        )
        self.pairsList = helper.addLabeledControl(
            listLabel, wx.ListBox, choices=[], style=wx.LB_SINGLE
        )
        ...
        mainSizer.Add(
            helper.sizer, border=gui.guiHelper.BORDER_FOR_DIALOGS, flag=wx.ALL
        )
        mainSizer.Fit(self)
    ...
    def refresh(self, selection):
        self.pairsList.Set(
            [slotLabel(index, pair) for index, pair in enumerate(self.pairs)]
        )
        if self.pairs:
...
class LanguagePairDialog(wx.Dialog):
    def __init__(self, parent, sourceLangs, targetLangs, pair=None):
        ...
        # Translators: A setting in addon settings dialog.
        self._fromChoice = helper.addLabeledControl(
            _("Source language:"), wx.Choice, choices=sourceChoices(sourceLangs)
        )
        # Translators: A setting in addon settings dialog.
        self._intoChoice = helper.addLabeledControl(
            _("Target language:"), wx.Choice, choices=list(targetLangs)
        )
        helper.addDialogDismissButtons(self.CreateButtonSizer(wx.OK | wx.CANCEL))
        mainSizer.Add(
            helper.sizer, border=gui.guiHelper.BORDER_FOR_DIALOGS, flag=wx.ALL
        ...
        self._fromChoice.Select(
            max(0, self._fromChoice.FindString(getName(sourceLangs, pair.langFrom)))
            if pair
            else 0
        )
        self._intoChoice.Select(
            max(0, self._intoChoice.FindString(getName(targetLangs, pair.langTo)))
            if pair
            else 0
        )
        self.CentreOnScreen()
