...
def executeWithSpeakOnDemand(f, *args, **kwargs):
    ...
    if threading.get_ident() != core.mainThreadId:
        raise RuntimeError(
            "This function should only be executed from the main thread."
        )

    if (
        not isSpeechOnDemandFeatureAvailable()
        or speech.getState().speechMode != speech.SpeechMode.onDemand
    ):
